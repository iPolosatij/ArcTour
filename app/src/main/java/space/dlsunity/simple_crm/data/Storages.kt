package space.dlsunity.simple_crm.data
...
class Storages {
        ...
        if (imageStoragePath == null) {
            ...
            if (outgoing) {
                storeFile = File(storeFile,
                    space.dlsunity.simple_crm.data.Storages.Companion.OUTGOING_PATH
                )
            }
            if (!storeFile.exists()) {
                // TODO: add error handling
    ...
    internal class StoragesBuilder {
        ...
        private var cachePath: String? = null
        private var filesDir: String? = null
        fun setExternalStoragePath(externalStoragePath: String?): space.dlsunity.simple_crm.data.Storages.StoragesBuilder {
            this.externalStoragePath = externalStoragePath
            return this
        }

        fun setCachePath(cachePath: String?): space.dlsunity.simple_crm.data.Storages.StoragesBuilder {
            this.cachePath = cachePath
            return this
        }

        fun setFilesDir(filesDir: String?): space.dlsunity.simple_crm.data.Storages.StoragesBuilder {
            this.filesDir = filesDir
            return this
        }

        fun build(): space.dlsunity.simple_crm.data.Storages {
            return space.dlsunity.simple_crm.data.Storages(externalStoragePath, cachePath, filesDir)
        }
    }

    companion object {
        private val TAG = space.dlsunity.simple_crm.data.Storages::class.java.simpleName
